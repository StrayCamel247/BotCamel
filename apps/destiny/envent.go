...
func (r *Destiny) groMsgHandler(com, content string) {
	...
	// 判断指令处理消息
	switch {
	case utils.EqualFolds(com, command.D2pvp.Key()):
		r.pvpInfoHandler(content)

	case utils.EqualFolds(com, command.D2pve.Key()):
		r.pveInfoHandler(content)

	case utils.EqualFolds(com, command.D2skill.Key()):
		r.GenerateDes(content, "skil")

	case utils.EqualFolds(com, command.D2npc.Key()):
		r.GenerateDes(content, "npc")

	case utils.EqualFolds(com, command.D2perk.Key()):
		r.GenerateDes(content, "perk")

	case utils.EqualFolds(com, command.D2item.Key()):
		r.ItemGenerateImg(content, "item")

	case utils.EqualFolds(com, command.D2day.Key()):
		r.dayGenerateImg("0x03")

	case utils.EqualFolds(com, command.D2week.Key()):
		r.d2uploadImgByFlag("0x02")

	case utils.EqualFolds(com, command.D2xiu.Key()):
		r.d2uploadImgByFlag("0x04")

	case utils.EqualFolds(com, command.D2trial.Key()):
		r.d2uploadImgByFlag("0x05")

	case utils.EqualFolds(com, command.D2dust.Key()):
		r.d2uploadImgByFlag("0x06")

	case utils.EqualFolds(com, command.D2random.Key()):
		r.randomHandler()
	case out == "":
